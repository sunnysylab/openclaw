...
- `MEMORY.md`（存在时；否则兼容 `memory.md` 小写回退）
...
子智能体会话仅注入 `AGENTS.md`、`OPENCLAW.md`（存在时）、`CLAUDE.md`（存在时）、`TOOLS.md`、`SOUL.md`、`IDENTITY.md` 和 `USER.md`，其他引导文件会被过滤以保持上下文更薄。

要检查每个注入文件贡献了多少（原始 vs 注入、截断、policy slicing、合并顺序/冲突提示，以及工具 schema 开销），使用 `/context list` 或 `/context detail`。参见[上下文](/concepts/context)。
