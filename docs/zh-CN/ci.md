...
| 任务             | 目的                                                                      | 运行时机                                         |
| ----------------- | ------------------------------------------------------------------------- | ------------------------------------------------ |
| `preflight`       | 文档范围、范围更改、密钥扫描、工作流审计、生产依赖审计 | 始终；仅非文档更改时进行基于节点的审计 |
...
同一个模块还驱动单独的 `install-smoke` 工作流，通过更窄的 `changed-smoke` 门，因此 Docker/安装烟雾测试只运行安装、打包和容器相关的更改。
