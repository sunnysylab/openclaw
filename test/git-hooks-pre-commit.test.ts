...
describe("git-hooks/pre-commit (integration)", () => {
  ...
  it("routes staged-file filtering through the helper instead of raw node on PATH", () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "openclaw-pre-commit-node-helper-"));
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);
    ...
    writeExecutable(
      fakeBinDir,
      "node",
      "#!/usr/bin/env bash\necho 'raw node should not be called by pre-commit' >&2\nexit 99\n",
    );
    ...
    const staged = run(dir, "git", ["diff", "--cached", "--name-only"]).split("\n").filter(Boolean);
    expect(staged).toEqual(["tracked.txt"]);
  });

  it("re-stages tracked ignored files with force", () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "openclaw-pre-commit-ignored-"));
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);
    ...
    mkdirSync(path.join(dir, "vendor", "a2ui"), { recursive: true });
    writeFileSync(path.join(dir, "vendor", "a2ui", "tracked.txt"), "kept\n", "utf8");
    run(dir, "git", ["add", "--", ".gitignore"]);
    run(dir, "git", ["add", "-f", "--", "vendor/a2ui/tracked.txt"]);
    ...
    const staged = run(dir, "git", ["diff", "--cached", "--name-only"]).split("\n").filter(Boolean);
    expect(staged).toEqual([".gitignore", "vendor/a2ui/tracked.txt"]);
  });
});
