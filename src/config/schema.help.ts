...
export const FIELD_HELP: Record<string, string> = {
  ...
  "mcp.servers.*.persistent":
    "When true, keeps the MCP server process alive at gateway level and shares it across all agent sessions. Only supported for stdio servers (requires command). Enables browser state, cookies, and login sessions to persist across conversations. Explicit opt-in — shared process state is visible to all sessions.",
  session:
