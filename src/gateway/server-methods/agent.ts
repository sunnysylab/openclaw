...
function setOwnedAbortControllerEntry(params: {
  ...
  const isSubagentLane =
    typeof params.lane === "string" && params.lane.trim() === String(AGENT_LANE_SUBAGENT);
  const timeoutSecondsRaw =
    params.timeout !== undefined
      ? Number.parseInt(String(params.timeout), 10)
      : isSubagentLane
        ? 0
        : undefined;
...
const INTERNAL_AGENT_RUN_SESSION_PREFIX = "__agent_run__:";

function resolveTrackedAgentSessionKey(runId: string, sessionKey?: string): string {
  return sessionKey ?? `${INTERNAL_AGENT_RUN_SESSION_PREFIX}${runId}`;
}
...
function dispatchAgentRunFromGateway(params: {
  ...
  const currentEntry = params.context.chatAbortControllers.get(params.runId);
  if (currentEntry && currentEntry.controller !== controller) {
    respondDispatchClaimFailure(
      errorShape(
  ...
  if (controller.signal.aborted) {
    ...
    respondDispatchClaimFailure(
      errorShape(
        ...
        `run "${params.runId}" was aborted before dispatch started`,
      ),
    );
    return false;
  }
  setOwnedAbortControllerEntry({
    ...
    timeout: params.ingressOpts.timeout,
    lane: params.ingressOpts.lane,
...
export const agentHandlers: GatewayRequestHandlers = {
  agent: async ({ params, respond, context, client, isWebchatConnect }) => {
    ...
    const claimedAbortController = new AbortController();
    let trackedSessionKey = resolveTrackedAgentSessionKey(runId, requestedSessionKey);
    ...
    const clearClaimedAbortControllerIfOwned = () => {
      if (context.chatAbortControllers.get(runId)?.controller === claimedAbortController) {
    ...
    // finishes, so it uses the current raw session key when one exists. For
    // sessionless agent runs we use an internal synthetic key so the shared
    // abort registry still protects the runId namespace and makes the in-flight
    // run discoverable to runtime.agent.abort().
    setOwnedAbortControllerEntry({
    ...
    try {
      ...
      if (requestedSessionKey) {
        ...
        trackedSessionKey = canonicalSessionKey;
      ...
      dispatchStarted = dispatchAgentRunFromGateway({
        ingressOpts: {
          message,
          images,
          imageOrder,
  ...
  "agent.wait": async ({ params, respond, context }) => {
    ...
    const timeoutMs =
      typeof p.timeoutMs === "number" && Number.isFinite(p.timeoutMs)
        ...
        : 30_000;
    const activeTrackedRun = context.chatAbortControllers.get(runId);
    const hasActiveChatRun = activeTrackedRun?.kind === "chat";
    const hasActiveTrackedRun = activeTrackedRun !== undefined;
    ...
    const lifecyclePromise = waitForAgentJob({
      ...
      // When any tracked run is currently active with the same runId, ignore
      // cached lifecycle snapshots so stale agent results do not preempt the
      // active run before its fresh lifecycle:start arrives.
      ignoreCachedSnapshot: hasActiveTrackedRun,
    });
