...
const CONTROL_UI_ASSETS_MISSING_MESSAGE =
  "Control UI assets not found. Build them with `pnpm ui:build` (auto-installs UI deps), or run `pnpm ui:dev` during development. If you just built the UI, restart the gateway and try again.";
...
function respondControlUiAssetsUnavailable(
  ...
  if (options?.configuredRootPath) {
    respondPlainText(
      res,
      503,
      `Control UI assets not found at ${options.configuredRootPath}. Build them with \`pnpm ui:build\` (auto-installs UI deps), or update gateway.controlUi.root. If you just built the UI, restart the gateway and try again.`,
    );
    return;
  }
