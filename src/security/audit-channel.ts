...
export async function collectChannelSecurityFindings(params: {
  ...
  for (const plugin of params.plugins) {
    ...
    for (const accountId of orderedAccountIds) {
      ...
      const {
        normalizeTelegramAllowFromEntry: normalizeTgEntry,
        isNumericTelegramUserId: isNumTg,
      } = await loadAuditChannelTelegramRuntimeModule();
      const validDefaultGroupAllowFrom = defaultGroupAllowFrom.filter((entry) => {
        const normalized = normalizeTgEntry(entry);
        return normalized !== null && (normalized === "*" || isNumTg(normalized));
      });
