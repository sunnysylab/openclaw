...
export async function loadModelCatalog(params?: {
  ...
  modelCatalogPromise = (async () => {
    ...
    try {
      ...
      try {
        const localGgufSource = new LocalGgufDiscoverySource();
        const localGgufModels = await localGgufSource.discover({
          config: cfg,
          env: process.env,
        });
        for (const entry of localGgufModels) {
          const id = String(entry?.id ?? "").trim();
          if (!id) {
            continue;
          }
        ...
        log.warn(`Local GGUF discovery failed: ${String(e)}`);
      }
