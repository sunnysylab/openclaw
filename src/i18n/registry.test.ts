...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["en", "zh-CN", "zh-TW", "pt-BR", "de", "es", "ko-KR"]);
  ...
  it("resolves browser locale fallbacks", () => {
    ...
    expect(resolveNavigatorLocale("ko-KR")).toBe("ko-KR");
    expect(resolveNavigatorLocale("ko")).toBe("ko-KR");
  });
  ...
  it("loads lazy locale translations from the registry", async () => {
    ...
    const koKR = await loadLazyLocaleTranslation("ko-KR");
    ...
    expect(getNestedTranslation(koKR, "common", "health")).toBe("상태");
    expect(getNestedTranslation(koKR, "languages", "koKR")).toBe("한국어");
