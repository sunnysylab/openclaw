...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["en", "zh-CN", "zh-TW", "pt-BR", "de", "es", "ko"]);
  ...
  it("loads lazy locale translations from the registry", async () => {
    ...
    const ko = await loadLazyLocaleTranslation("ko");
    expect(getNestedTranslation(ko, "common", "health")).toBe("\uc0c1\ud0dc"); // Health in Korean according to ko.ts
