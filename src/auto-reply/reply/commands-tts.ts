...
function ttsUsage(): ReplyPayload {
  ...
  return {
    text:
      ...
      `• deepgram — Aura voices (requires API key)\n\n` +
...
export const handleTtsCommands: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (action === "provider") {
    ...
    if (!args.trim()) {
      ...
      const hasDeepgram = Boolean(resolveTtsApiKey(config, "deepgram"));
      return {
        shouldContinue: false,
        reply: {
          text:
            ...
            `Deepgram key: ${hasDeepgram ? "✅" : "❌"}\n` +
            `Usage: /tts provider openai | elevenlabs | edge | deepgram`,
        },
      };
    }
