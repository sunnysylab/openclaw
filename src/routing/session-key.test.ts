...
describe("getSubagentDepth", () => {
  ...
  it("returns 1 for legacy subagent session keys", () => {
    expect(getSubagentDepth("subagent:worker")).toBe(1);
  });

  it("returns 2 for nested legacy subagent session keys", () => {
    expect(getSubagentDepth("subagent:parent:subagent:child")).toBe(2);
  });
