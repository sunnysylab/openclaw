...
export async function noteMemorySearchHealth(
  ...
  for (const provider of ["openai", "gemini", "voyage", "mistral", "kilocode"] as const) {
  ...
  note(
    [
      ...
      "",
      "Fix (pick one):",
      "- Set OPENAI_API_KEY, GEMINI_API_KEY, VOYAGE_API_KEY, MISTRAL_API_KEY, or KILOCODE_API_KEY in your environment",
      `- Configure credentials: ${formatCliCommand("openclaw configure --section model")}`,
...
async function hasApiKeyForProvider(
  provider: "openai" | "gemini" | "voyage" | "mistral" | "ollama" | "kilocode",
  cfg: OpenClawConfig,
  agentDir: string,
): Promise<boolean> {
