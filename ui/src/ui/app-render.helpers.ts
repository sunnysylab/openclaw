...
/**
 * Returns true when a displayName looks like an auto-generated group name from
 * buildGroupDisplayName (e.g. "webchat:g-agent-main-abc123").  These internal
 * identifiers are unreadable and should not be surfaced directly in the UI.
 */
function isAutoGeneratedGroupDisplayName(name: string): boolean {
  return /^[a-z][a-z0-9-]*:g-/.test(name);
}

/**
 * Extract a short (≤8 char) human-readable ID from a session key.
 * Prefers the first hex block of a UUID segment if present.
 */
function extractShortSessionId(key: string): string {
  const uuidMatch = key.match(/([0-9a-f]{8})-[0-9a-f]{4}-[0-9a-f]{4}/i);
  if (uuidMatch) {
    return uuidMatch[1];
  }
...
export function resolveSessionDisplayName(
  ...
  // Skip auto-generated group display names (e.g. "webchat:g-agent-main-<uuid>").
  // They are internal identifiers; prefer the parsed fallback or a short ID.
  if (displayName && displayName !== key && !isAutoGeneratedGroupDisplayName(displayName)) {
    return applyTypedPrefix(displayName);
  }
  // For subagent sessions with no useful label/displayName, show a short ID
  // derived from the UUID in the session key (e.g. "Subagent: 4f2146de").
  if (prefix === "Subagent:" && fallbackName === "Subagent:") {
    const shortId = extractShortSessionId(key);
    return shortId ? `${prefix} ${shortId}` : fallbackName;
  }
  return fallbackName;
}
