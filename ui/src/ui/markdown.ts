...
htmlEscapeRenderer.image = (token: { href?: string | null; text?: string | null }) => {
  ...
  // Allow inline base64 data URIs (e.g. charts generated by LLMs).
  if (INLINE_DATA_IMAGE_RE.test(href)) {
    return `<img class="markdown-inline-image" src="${escapeHtml(href)}" alt="${escapeHtml(label)}">`;
  }

  // Allow remote http/https images; load lazily to avoid blocking the page.
  if (REMOTE_IMAGE_URL_RE.test(href)) {
    return `<img class="markdown-inline-image" src="${escapeHtml(href)}" alt="${escapeHtml(label)}" loading="lazy">`;
  }

  // All other schemes (javascript:, data: with non-image MIME, etc.) are unsafe — degrade to alt text.
