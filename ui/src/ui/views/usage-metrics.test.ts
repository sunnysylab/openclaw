...
describe("buildPeakErrorHours", () => {
  ...
  it("shifts UTC quarter-hour buckets to local timezone in local mode", () => {
    // Simulate UTC+5 (offset = -300 min → getTimezoneOffset() returns -300)
    vi.spyOn(Date.prototype, "getTimezoneOffset").mockReturnValue(-300);

    // quarterIndex 0 → utcMinute 0 → localMinute = 0 - (-300) = 300 → hour 5
    // quarterIndex 40 → utcMinute 600 → localMinute = 600 - (-300) = 900 → hour 15
    const session = makeSessionWithQuarterHourly([
    ...
    expect(subs).toContain("3 errors · 10 msgs"); // hour 5
    expect(subs).toContain("4 errors · 20 msgs"); // hour 15
  });

  it("wraps correctly for negative local timezone (UTC-8)", () => {
    // Simulate UTC-8 (offset = +480 min → getTimezoneOffset() returns 480)
    vi.spyOn(Date.prototype, "getTimezoneOffset").mockReturnValue(480);

    // quarterIndex 0 → utcMinute 0 → localMinute = 0 - 480 = -480
    //   → hour = floor(-480/60) % 24 = -8 % 24 → ((-8 % 24) + 24) % 24 = 16
    const session = makeSessionWithQuarterHourly([
      { date: "2026-03-15", quarterIndex: 0, total: 10, errors: 5 },
    ]);
  ...
  it("wraps correctly for positive local timezone near midnight (UTC+8, late quarter)", () => {
    ...
    // quarterIndex 68 → utcMinute = 68*15 = 1020 (17:00 UTC)
    //   → localMinute = 1020 - (-480) = 1500 → hour = floor(1500/60) = 25 → 25 % 24 = 1
    const session = makeSessionWithQuarterHourly([
