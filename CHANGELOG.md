...
- Agents/Anthropic: split the system prompt into a cached static prefix and uncached dynamic suffix on direct Anthropic and Bedrock Anthropic paths, so prompt caching survives per-turn system changes instead of rewriting the full prefix every turn. Thanks @coletebou. Maintainer refresh: vincentkoc.
