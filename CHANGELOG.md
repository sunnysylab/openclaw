...
- Telegram/approvals: route plugin approval requested/resolved events through the Telegram exec approval handler, keep the same inline approval buttons as exec approvals, wait for the gateway hello before the first direct callback resolve, preserve `allow-always` callback routing plus explicit approver identity on that direct gateway path, and remove the Telegram-side session-target cast by widening the shared target helper to the common request shape. Thanks @nk3750 and @vincentkoc.
