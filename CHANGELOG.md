...
- CLI/dotenv: block workspace `.env` files from overriding pinned helper interpreter selectors, so repo-local dotenv files can no longer redirect filesystem-safe helper execution.
