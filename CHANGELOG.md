...
- Agents/exec silent-failure fallback: when `exec`/`bash` warnings are suppressed in normal mode and a turn would otherwise end without any user-facing reply, emit a generic failure message instead of leaving channel threads silent.
