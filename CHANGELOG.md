...
=======
- Skills: add bundled `football-match-analyst` skill with modular `match-context.mjs` (`lib/api-football.mjs`, `lib/sportmonks.mjs`, `lib/nami.mjs`, `lib/shared.mjs`): `llmPack` aggregates, optional `--last` up to 100, last-5 xG/possession, odds, schedule load, `--fixture` mode; `--provider sportmonks` uses `SPORTMONKS_TOKEN` / `SPORTMONKS_API_TOKEN`; `--provider nami` uses `NAMI_USER` / `NAMI_SECRET` (Nami 纳米数据) with configurable `NAMI_PATH_*` HTTP paths.
- Skills: `football-match-analyst` adds `--provider opta` (`statsperform` alias) via `lib/opta.mjs`: contract-configurable Opta/Stats Perform REST (`OPTA_API_BASE`, `OPTA_API_KEY`, `OPTA_PATH_*`, `OPTA_AUTH_MODE`, optional team-id env); `llmPack.dataSource` = `opta`.
- Skills: `football-match-analyst` adds `--providers` multi-provider mode (`lib/run-providers.mjs`): parallel runs, `bySource` + optional `--primary-provider`; OpenClaw `skills.entries` can supply all keys per enabled source.
- Skills: add bundled `football-live-commentary` with `scripts/opta-live-pack.mjs` (`lib/opta-event-classifier.mjs`, `lib/opta-commentary-pack.mjs`): Opta-like event JSON/NDJSON → `commentaryPack` (high/medium/low filtering, match context, persona guides: neutral/data/passion/poetic); add `scripts/nami-live-pack.mjs` + `lib/nami-live-normalize.mjs` / `lib/nami-live-fetch.mjs` for 纳米足球实时数据 ([doc](https://www.nami.com/zh/details/j3ry6iztqltnwe0)), CJK hint augmentation in classifier, optional `--fetch` with `NAMI_*` env.
- Skills: add bundled `football-local-data` (skill id `football_local_data`) with `scripts/local-data-fetch.mjs` and pluggable `scripts/sources/*`: default **`football-data-co-uk`** ([Football-Data.co.uk](https://www.football-data.co.uk/) CSVs; `--preset` bundles including `all`, `--season-range`, `--seasons`, `manifest.json`); optional **`http-csv`** (`--urls`); **`fbref`** + `scripts/fbref-fetch.py` + `requirements-fbref.txt` ([soccerdata](https://soccerdata.readthedocs.io/) → [FBref](https://fbref.com/), `--fbref-depth`, `--preset all` / `--leagues ALL`); **`--sources`** multi-fetch into `OUT/<source_id>/`; multi-season under `OUT/<season>/` + `index.json`; legacy `scripts/football-data-fetch.mjs` — not betting advice.
- Skills: `football-local-data` **`fbref`** adds **`--fbref-proxy`** (and **`fbref-fetch.py` `--proxy`**) plus env **`HTTPS_PROXY`** / **`FBREF_PROXY`** / **`ALL_PROXY`** for soccerdata outbound HTTP(S) via a local proxy.
- Skills: **`football-match-analyst`** adds **`scripts/kaggle-fetch.mjs`** + **`scripts/kaggle-list-mine-refs.py`** + **`requirements-kaggle.txt`**: Kaggle API downloads via **`KAGGLE_USERNAME`** / **`KAGGLE_KEY`** (or **`~/.kaggle/kaggle.json`**), then use **`--provider football-data --csv`**; **`--mine`** batch-downloads datasets owned by the account.
- Skills: `football-match-analyst` adds `--provider football-data` (`lib/football-data-csv.mjs`): read local Football-Data.co.uk CSV into the same `llmPack` shape (`--csv` or `FOOTBALL_DATA_CSV` / `FOOTBALL_DATA_CSV_PATH`); optional `--fixture` row index (`csv-N` ids).
>>>>>>> 96d93d7383 (Skills: add football-live-commentary, football-local-data, football-match-analyst)
