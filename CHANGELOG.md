...
- Tool policies: preserve additive `alsoAllow` behavior for shared tool-policy resolution, keep optional plugin/tool opt-ins visible under allow-all configs, and make plugin-only allowlist warnings stop mislabeling gated core tools or typos as plugin issues.
