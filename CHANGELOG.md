...
- Tailscale/runtime: block workspace `.env` from setting the test binary override and restrict that override to explicit test environments, so repo-local dotenv files can no longer redirect runtime Tailscale commands.
