...
describe("AcpxRuntime", () => {
  ...
  it("clears stale setup errors after recovery and surfaces later doctor failures", async () => {
    const previousStdout = process.env.MOCK_ACPX_HELP_STDOUT;
    const previousExitCode = process.env.MOCK_ACPX_HELP_EXIT_CODE;
    ...
    try {
      ...
      runtime.setSetupError("startup install failed");
      ...
      process.env.MOCK_ACPX_HELP_STDOUT = "probe failed after recovery";
      process.env.MOCK_ACPX_HELP_EXIT_CODE = "1";

      await expect(runtime.doctor()).resolves.toMatchObject({
        ok: false,
        code: "ACP_BACKEND_UNAVAILABLE",
        message: expect.stringContaining("probe failed after recovery"),
      });
      expect(runtime.isHealthy()).toBe(false);
      expect(runtime.getUnhealthyReason()).toContain("probe failed after recovery");
    } finally {
      if (previousStdout === undefined) {
        delete process.env.MOCK_ACPX_HELP_STDOUT;
      } else {
