...
async function loadSubject() {
  ...
  vi.doMock("openclaw/plugin-sdk/reply-runtime", () => ({
    ...
    resolveHeartbeatReplyPayload: (reply: { text?: string } | undefined) => reply,
    stripHeartbeatToken: (text: string | undefined) => {
      const normalized = (text ?? "").trim();
      if (normalized === HEARTBEAT_TOKEN) {
        return { shouldSkip: true, text: "" };
      }
      return { shouldSkip: false, text: text ?? "" };
    },
  }));
  ...
  return import("./heartbeat-runner.js");
}
...
describe("runWebHeartbeatOnce", () => {
  ...
  it("supports manual override body dry-run without sending", async () => {
    const { runWebHeartbeatOnce } = await loadSubject();
