...
function resolveMiniMaxApiKey(searchConfig?: SearchConfigRecord): string | undefined {
  return (
    ...
    readProviderEnvValue(["MINIMAX_CODE_PLAN_KEY", "MINIMAX_API_KEY"])
  );
}
...
function resolveMiniMaxRegion(
  ...
  // 2. Infer from model provider base URL (set by CN onboarding)
  ...
  if (baseUrl.includes("minimaxi.com") || portalBaseUrl.includes("minimaxi.com")) {
    return "cn";
  }
...
function missingMiniMaxKeyPayload() {
  return {
    ...
    message: `web_search (minimax) needs a MiniMax API key. Run \`${formatCliCommand("openclaw configure --section web")}\` to store it, or set MINIMAX_API_KEY or MINIMAX_CODE_PLAN_KEY in the Gateway environment.`,
...
export function createMiniMaxWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    envVars: ["MINIMAX_CODE_PLAN_KEY", "MINIMAX_API_KEY"],
    placeholder: "sk-...",
