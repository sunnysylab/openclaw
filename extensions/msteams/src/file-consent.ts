...
export async function validateConsentUploadUrl(
  ...
  const resolveFn = opts?.resolveFn ?? lookup;
  let resolved: { address: string };
  try {
    resolved = await resolveFn(hostname);
  } catch {
    throw new Error(`Failed to resolve consent upload URL hostname "${hostname}"`);
  }

  if (isPrivateOrReservedIP(resolved.address)) {
    throw new Error(`Consent upload URL resolves to a private/reserved IP (${resolved.address})`);
  }
}
