...
import {
  ...
  resolveFeishuToolAccountFromContext,
...
export function registerFeishuDocTools(api: OpenClawPluginApi) {
  ...
  const getMediaMaxBytes = (
    ...
    (resolveFeishuToolAccountFromContext({ api, executeParams: params, toolContext }).config
      ?.mediaMaxMb ?? 30) *
    1024 *
    1024;
      ...
      (ctx) => {
        ...
        return {
          ...
          async execute(_toolCallId, params) {
            ...
            try {
              const client = getClient(p, toolContext);
              switch (p.action) {
                case "read":
                ...
                case "write":
                  return json(
                    await writeDoc(
                      ...
                      p.doc_token,
                      p.content,
                      getMediaMaxBytes(p, toolContext),
                      api.logger,
                    ),
                  );
                case "append":
