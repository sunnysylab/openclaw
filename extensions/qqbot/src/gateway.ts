...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  if (savedSession) {
    ...
    log?.info(
      `[qqbot:${account.accountId}] Restored session from storage: sessionId=${sessionId}, lastSeq=${lastSeq}, intentLevel=${intentLevelIndex}`,
    );
  }
  ...
  const enqueueMessage = (msg: QueuedMessage): void => {
    ...
    if (queue.length >= PER_USER_QUEUE_SIZE) {
      ...
      log?.error(
        `[qqbot:${account.accountId}] Per-user queue full for ${peerId}, dropping oldest message ${dropped?.messageId}`,
      );
    }
    ...
    if (totalEnqueued > MESSAGE_QUEUE_SIZE) {
      log?.error(
        `[qqbot:${account.accountId}] Global queue limit reached (${totalEnqueued}), message from ${peerId} may be delayed`,
      );
    }

    queue.push(msg);
    log?.debug?.(
      `[qqbot:${account.accountId}] Message enqueued for ${peerId}, user queue: ${queue.length}, active users: ${activeUsers.size}`,
    );
  ...
  const drainUserQueue = async (peerId: string): Promise<void> => {
    ...
    if (activeUsers.size >= MAX_CONCURRENT_USERS) {
      log?.info(
        `[qqbot:${account.accountId}] Max concurrent users (${MAX_CONCURRENT_USERS}) reached, ${peerId} will wait`,
      );
        ...
        if (event.attachments?.length) {
          ...
          for (const att of event.attachments) {
            ...
            if (isVoice && att.voice_wav_url) {
              const wavUrl = att.voice_wav_url.startsWith("//")
              ...
              if (wavLocalPath) {
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Voice attachment: ${att.filename}, downloaded WAV directly (skip SILK→WAV)`,
                );
              } else {
                log?.error(
                  `[qqbot:${account.accountId}] Failed to download voice_wav_url, falling back to original URL`,
                );
              }
            }
            ...
            if (localPath) {
              if (att.content_type?.startsWith("image/")) {
                ...
                if (!sttCfg) {
                  log?.info(
                    `[qqbot:${account.accountId}] Voice attachment: ${att.filename} (STT not configured, skipping transcription)`,
                  );
                  ...
                  if (!audioPath) {
                    ...
                    log?.info(
                      `[qqbot:${account.accountId}] Voice attachment: ${att.filename}, converting SILK→WAV...`,
                    );
                    try {
                      ...
                      if (wavResult) {
                        ...
                        log?.info(
                          `[qqbot:${account.accountId}] Voice converted: ${wavResult.wavPath} (${formatDuration(wavResult.duration)})`,
                        );
                      } else {
                      ...
                      log?.error(
                        `[qqbot:${account.accountId}] Voice conversion failed: ${convertErr}`,
                      );
                  ...
                  try {
                    ...
                    if (transcript) {
                      log?.info(
                        `[qqbot:${account.accountId}] STT transcript: ${transcript.slice(0, 100)}...`,
                      );
            ...
            pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
              ...
              dispatcherOptions: {
                ...
                deliver: async (
                  ...
                  if (info.kind === "tool") {
                    log?.info(
                      `[qqbot:${account.accountId}] Skipping tool result deliver (intermediate, not user-facing)`,
                    );
                    return;
                  }
                  ...
                  if (mediaTagMatches.length > 0) {
                    const imgCount = mediaTagMatches.filter(
                      (m) => m[1]!.toLowerCase() === "qqimg",
                    ).length;
                    const voiceCount = mediaTagMatches.filter(
                      (m) => m[1]!.toLowerCase() === "qqvoice",
                    ).length;
                    const videoCount = mediaTagMatches.filter(
                      (m) => m[1]!.toLowerCase() === "qqvideo",
                    ).length;
                    const fileCount = mediaTagMatches.filter(
                      (m) => m[1]!.toLowerCase() === "qqfile",
                    ).length;
                    log?.info(
                      `[qqbot:${account.accountId}] Detected media tags: ${imgCount} <qqimg>, ${voiceCount} <qqvoice>, ${videoCount} <qqvideo>, ${fileCount} <qqfile>`,
                    );
                    ...
                    while ((match = mediaTagRegexWithIndex.exec(replyText)) !== null) {
                      ...
                      try {
                        ...
                        if (hasOctal || hasNonASCII) {
                          log?.debug?.(
                            `[qqbot:${account.accountId}] Decoding path with mixed encoding: ${mediaPath}`,
                          );
                          ...
                          if (
                            ...
                            log?.debug?.(
                              `[qqbot:${account.accountId}] Successfully decoded path: ${mediaPath}`,
                            );
                          }
                        }
                      } catch (decodeErr) {
                      ...
                      if (mediaPath) {
                        if (tagName === "qqvoice") {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Found voice path in <qqvoice>: ${mediaPath}`,
                          );
                        } else if (tagName === "qqvideo") {
                          sendQueue.push({ type: "video", content: mediaPath });
                          log?.info(
                            `[qqbot:${account.accountId}] Found video URL in <qqvideo>: ${mediaPath}`,
                          );
                        } else if (tagName === "qqfile") {
                          sendQueue.push({ type: "file", content: mediaPath });
                          log?.info(
                            `[qqbot:${account.accountId}] Found file path in <qqfile>: ${mediaPath}`,
                          );
                        } else {
                          sendQueue.push({ type: "image", content: mediaPath });
                          log?.info(
                            `[qqbot:${account.accountId}] Found image path in <qqimg>: ${mediaPath}`,
                          );
                        }
                      }
                    ...
                    log?.info(
                      `[qqbot:${account.accountId}] Send queue: ${sendQueue.map((item) => item.type).join(" -> ")}`,
                    );
                    ...
                    for (const item of sendQueue) {
                      if (item.type === "text") {
                        ...
                        try {
                          ...
                          if (isLocalPath) {
                            ...
                            if (!(await fileExistsAsync(imagePath))) {
                              log?.error(
                                `[qqbot:${account.accountId}] Image file not found: ${imagePath}`,
                              );
                            ...
                            if (!mimeType) {
                              log?.error(
                                `[qqbot:${account.accountId}] Unsupported image format: ${ext}`,
                              );
                            ...
                            log?.error(
                              `[qqbot:${account.accountId}] Invalid image path (not local or URL): ${imagePath}`,
                            );
                            continue;
                          }
                          ...
                          await sendWithTokenRetry(async (token) => {
                            if (event.type === "c2c") {
                              ...
                              if (isHttpUrl) {
                                ...
                                // 频道不支持富媒体 Base64
                                log?.info(
                                  `[qqbot:${account.accountId}] Channel does not support rich media for local images`,
                                );
                              }
                            }
                          });
                          log?.info(
                            `[qqbot:${account.accountId}] Sent image via <qqimg> tag: ${imagePath.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.error(
                        ...
                        try {
                          ...
                          if (fileSize === 0) {
                            log?.error(
                              `[qqbot:${account.accountId}] Voice file not ready after waiting: ${voicePath}`,
                            );
                          ...
                          if (!silkBase64) {
                            ...
                            log?.error(
                              `[qqbot:${account.accountId}] Voice conversion to SILK failed: ${ext} (${fileSize} bytes). Check [audio-convert] logs for details.`,
                            );
                            await sendErrorMessage(`语音格式转换失败，请稍后重试`);
                            continue;
                          }
                          log?.info(
                            `[qqbot:${account.accountId}] Voice file converted to SILK Base64 (${fileSize} bytes)`,
                          );
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Sent voice via <qqvoice> tag: ${voicePath.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.error(
                            `[qqbot:${account.accountId}] Failed to send voice from <qqvoice>: ${err}`,
                          );
                        ...
                        try {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Sent video via <qqvideo> tag: ${videoPath.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.error(
                            `[qqbot:${account.accountId}] Failed to send video from <qqvideo>: ${err}`,
                          );
                        ...
                        try {
                          ...
                          await sendWithTokenRetry(async (token) => {
                            if (isHttpUrl) {
                              ...
                              log?.info(
                                `[qqbot:${account.accountId}] Read local file (${formatFileSize(fileBuffer.length)}): ${filePath}`,
                              );
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Sent file via <qqfile> tag: ${filePath.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.error(
                            `[qqbot:${account.accountId}] Failed to send file from <qqfile>: ${err}`,
                          );
                  ...
                  if (payloadResult.isPayload) {
                    if (payloadResult.error) {
                      // 载荷解析失败，发送错误提示
                      log?.error(
                        `[qqbot:${account.accountId}] Payload parse error: ${payloadResult.error}`,
                      );
                          ...
                          try {
                            ...
                            if (!ttsText?.trim()) {
                              ...
                              if (!ttsCfg) {
                                ...
                                await sendErrorMessage(
                                  `[QQBot] TTS 未配置，请在 openclaw.json 的 channels.qqbot.tts 中配置`,
                                );
                              } else {
                                log?.info(
                                ...
                                const { silkBase64, duration } = await textToSilk(
                                  ttsText,
                                  ttsCfg,
                                  ttsDir,
                                );
                                log?.info(
                                  `[qqbot:${account.accountId}] TTS done: ${formatDuration(duration)}, uploading voice...`,
                                );
                            ...
                            log?.error(
                              `[qqbot:${account.accountId}] TTS/voice send failed: ${err}`,
                            );
                          ...
                          try {
                            ...
                            if (!videoPath?.trim()) {
                              ...
                              const isHttpUrl =
                                videoPath.startsWith("http://") || videoPath.startsWith("https://");
                              log?.info(
                                `[qqbot:${account.accountId}] Video send: "${videoPath.slice(0, 60)}..."`,
                              );
                  ...
                  const collectImageUrl = (url: string | undefined | null): boolean => {
                    ...
                    if (isHttpUrl || isDataUrl) {
                      if (!imageUrls.includes(url)) {
                        ...
                        if (isDataUrl) {
                          log?.info(
                            `[qqbot:${account.accountId}] Collected Base64 image (length: ${url.length})`,
                          );
                        } else {
                          log?.info(
                            `[qqbot:${account.accountId}] Collected media URL: ${url.slice(0, 80)}...`,
                          );
                        }
                      }
                      return true;
                    ...
                    if (isLocalFilePath(url)) {
                      ...
                      log?.info(
                        `[qqbot:${account.accountId}] 💡 Local path detected in non-structured message (not sending): ${url}`,
                      );
                      log?.info(
                        `[qqbot:${account.accountId}] 💡 Hint: Use <${suggestedTag}>${url}</${suggestedTag}> tag to send local ${mediaDesc}`,
                      );
                    }
                    return false;
                  };
                  ...
                  for (const match of mdMatches) {
                    ...
                    if (url && !imageUrls.includes(url)) {
                      if (url.startsWith("http://") || url.startsWith("https://")) {
                        ...
                        imageUrls.push(url);
                        log?.info(
                          `[qqbot:${account.accountId}] Extracted HTTP image from markdown: ${url.slice(0, 80)}...`,
                        );
                  ...
                  for (const match of bareUrlMatches) {
                    ...
                    if (url && !imageUrls.includes(url)) {
                      imageUrls.push(url);
                      log?.info(
                        `[qqbot:${account.accountId}] Extracted bare image URL: ${url.slice(0, 80)}...`,
                      );
                    }
                  }
                  ...
                  log?.info(
                    `[qqbot:${account.accountId}] Markdown mode: ${useMarkdown}, images: ${imageUrls.length}`,
                  );
                  ...
                  if (useMarkdown) {
                    ...
                    log?.info(
                      `[qqbot:${account.accountId}] Image classification: httpUrls=${httpImageUrls.length}, base64=${base64ImageUrls.length}`,
                    );

                    // 🔹 第一步：通过富媒体 API 发送 Base64 图片（本地文件已转换为 Base64）
                    if (base64ImageUrls.length > 0) {
                      log?.info(
                        `[qqbot:${account.accountId}] Sending ${base64ImageUrls.length} image(s) via Rich Media API...`,
                      );
                        ...
                        try {
                          await sendWithTokenRetry(async (token) => {
                            if (event.type === "c2c") {
                              ...
                              // 频道暂不支持富媒体，跳过
                              log?.info(
                                `[qqbot:${account.accountId}] Channel does not support rich media, skipping Base64 image`,
                              );
                            }
                          });
                          log?.info(
                            `[qqbot:${account.accountId}] Sent Base64 image via Rich Media API (size: ${imageUrl.length} chars)`,
                          );
                        } catch (imgErr) {
                          log?.error(
                    ...
                    for (const url of httpImageUrls) {
                      if (!existingMdUrls.has(url)) {
                        ...
                        try {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Formatted HTTP image: ${size ? `${size.width}x${size.height}` : "default size"} - ${url.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.info(
                    ...
                    for (const match of mdMatches) {
                      ...
                      if (isHttpUrl && !hasQQBotImageSize(fullMatch)) {
                        try {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Updated image with size: ${size ? `${size.width}x${size.height}` : "default"} - ${imgUrl.slice(0, 60)}...`,
                          );
                        } catch (err) {
                          log?.info(
                    ...
                    if (textWithoutImages.trim()) {
                      try {
                        ...
                        log?.info(
                          `[qqbot:${account.accountId}] Sent markdown message with ${httpImageUrls.length} HTTP images (${event.type})`,
                        );
                      } catch (err) {
                        log?.error(
                    ...
                    try {
                      ...
                      for (const imageUrl of imageUrls) {
                        try {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Sent image via media API: ${imageUrl.slice(0, 80)}...`,
                          );
                        } catch (imgErr) {
                          log?.error(
            ...
            case 9: // Invalid Session
              ...
              if (!canResume) {
                ...
                if (intentLevelIndex < INTENT_LEVELS.length - 1) {
                  ...
                  log?.info(
                    `[qqbot:${account.accountId}] Downgrading intents to: ${nextLevel.description}`,
                  );
                } else {
                  // 已经是最低权限级别了
                  log?.error(
                    `[qqbot:${account.accountId}] All intent levels failed. Please check AppID/Secret.`,
                  );
